...
3. **Multicollinearity Elimination:** Calculated a Pearson correlation matrix to identify redundant sensors. Found a 0.88 correlation between Air and Process temperatures and dropped the latter to reduce algorithmic noise.
...
4. **Algorithmic Rebalancing (SMOTE):** Utilized Synthetic Minority Over-sampling Technique (SMOTE) to mathematically synthesize failure data, teaching the AI the patterns of a breakdown without duplicating existing rows.

5. **Explainable AI:** Extracted feature importances from the Random Forest classifier to provide actionable business intelligence on which sensors indicate the highest risk.
